...
const BUILD_VERSION = "Zero Phase v0.2.0.7";
...
export default function Page() {
  ...
  const performDig = async (campaign: Campaign) => {
    ...
    const bar = (pct: number) => {
      ...
      const pctSafe = Number.isFinite(pctN) ? Math.max(0, Math.min(100, pctN)) : 0;

      const filled = Math.max(0, Math.min(blocks, Math.round((pctSafe / 100) * blocks)));
      const empty = Math.max(0, blocks - filled);

      return `[${"#".repeat(filled)}${"-".repeat(empty)}] ${pctSafe}%`;
    };
  ...
  const runCommand = (raw: string) => {
    ...
    // âœ… telegram verify (links TG user to this Terminal Pass)
