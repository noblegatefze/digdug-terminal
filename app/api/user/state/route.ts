...
export async function POST(req: NextRequest) {
    try {
        ...
        if (userErr) {
            return NextResponse.json({ ok: false, error: "user_lookup_failed", detail: userErr.message }, { status: 500 });
        }
        if (!user?.id) {
        ...
        if (body?.fuel) {
            ...
            if (updErr) {
                return NextResponse.json({ ok: false, error: "state_update_failed", detail: updErr.message }, { status: 500 });
            }
        }
        ...
        if (stateErr) {
            return NextResponse.json({ ok: false, error: "state_read_failed", detail: stateErr.message }, { status: 500 });
        }
