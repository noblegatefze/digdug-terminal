...
type Body = {
  ...
  group_key?: string; // "CHAIN|SYMBOL|TOKEN_ADDRESS"
  amount?: number;
  to_address?: string;
};
...
export async function POST(req: Request) {
  ...
  if (!group_key) return NextResponse.json({ ok: false, error: "missing_group_key" }, { status: 400 });
  ...
  // Parse key
  const [chain_id, token_symbol, token_address] = group_key.split("|");
  if (!chain_id || !token_symbol || !token_address) {
    return NextResponse.json({ ok: false, error: "invalid_group_key" }, { status: 400 });
  }

  const tx = `0x${randomBytes(32).toString("hex")}`;

  const { data, error } = await sb.rpc("rpc_terminal_withdraw_treasure_fifo", {
    p_user_id: u.id,
    p_username: username,
    p_chain_id: chain_id,
